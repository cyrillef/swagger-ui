...
function handleLogin() {
    ...
    popupDialog.find('button.api-popup-authbtn').click(function() {
        ...
        for (var key in authSchemes) {
            if (authSchemes.hasOwnProperty(key) && OAuthSchemeKeys.indexOf(key) != -1) { //only look at keys that match this scope.
                ...
                if(authSchemes[key].type === 'oauth2' && flow && (flow === 'implicit' || flow === 'accessCode')) {
                    ...
                    var client_id =this.authsCollectionView.collection.models [0].attributes.client_id ;
                    window.swaggerUi.client_id =(client_id || clientId) ;
                    var client_secret =this.authsCollectionView.collection.models [0].attributes.client_secret ;
                    window.swaggerUi.client_secret =(client_secret || clientSecret) ;
        ...
        var client_id =this.authsCollectionView.collection.models [0].attributes.client_id ;
        url +='&client_id=' + encodeURIComponent (client_id || clientId) ;
...
function clientCredentialsFlow(scopes, tokenUrl, OAuthSchemeKey) {
    var params = {
        'client_id': /*this.authsCollectionView.collection.models [0].attributes.client_id |*/ clientId,
        'client_secret': /*this.authsCollectionView.collection.models [0].attributes.client_secret |*/ clientSecret,
        'scope': scopes.join (' '),
...
window.processOAuthCode = function processOAuthCode(data) {
    ...
    var params = {
        'client_id': window.swaggerUi.client_id || clientId,
        'code': data.code,
        'grant_type': 'authorization_code',
        'redirect_uri': redirectUrl
    };

    if ( window.swaggerUi.client_secret || clientSecret ) {
        params.client_secret =window.swaggerUi.client_secret || clientSecret ;
    }

    params.url =window.swaggerUi.tokenUrl ;
    var tokenUrl =window.location.origin + '/token' ;

    $.ajax(
        {
            url : tokenUrl, //window.swaggerUi.tokenUrl,
            type: "POST",
            data: params,
            success:function(data, textStatus, jqXHR)
            {
